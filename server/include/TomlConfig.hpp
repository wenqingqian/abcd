...
class config : public noncopyable_toml_config{
private:
...
public:
	...
	struct config_ {
		...
		std::optional<T> value(){
			...
			// if constexpr ( same_type<T,Vector> ){
...
private:
	...
	struct parse_info: public noncopyable_toml_config{
		...
		//* 不实现为单例, 不然还要主动释放
		// static parse_info& getInstance(){
		// 	static parse_info pinfo;
		// 	return pinfo;
		// }
...
//!
//*解析文件
inline void config::parse(const string& path){
